...
import np.com.percoid.mvvmsampleapp.databinding.ProjectListItemBinding;
...
public class ProjectAdapter extends RecyclerView.Adapter<ProjectAdapter.ProjectViewHolder> {
    ...
    public void setProjectList(final List<? extends Project> projectsList){
        if(this.projectList == null){
            this.projectList = projectsList;
            notifyItemRangeChanged(0,projectsList.size());
        }else {
            DiffUtil.DiffResult result = DiffUtil.calculateDiff(new DiffUtil.Callback() {
                ...
                public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                    return ProjectAdapter.this.projectList.get(oldItemPosition).id == projectsList.get(newItemPosition).id;
                }

                @Override
                public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
                    Project project = projectsList.get(newItemPosition);
                    Project old = projectsList.get(oldItemPosition);
